...
const AddToWalletButton: React.FC<AddToWalletButtonProps & ButtonProps> = ({
  ...
  return (
    <Button
      {...props}
      onClick={() => {
        const tokenLogoURI = tokenLogo ?? getTokenLogoURLWithSymbol(tokenSymbol)
        const image = tokenLogoURI ? (BAD_SRCS[tokenLogoURI] ? undefined : tokenLogoURI) : undefined

        if (!tokenAddress || !tokenSymbol) return // || !image
