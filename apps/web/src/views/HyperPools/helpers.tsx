...
export const isEnded = (endTime) => {
  return endTime < Date.now() / 1000
}

export const isEnableToDeposit = (endTime, depositEndTime): boolean => {
  if (isEnded(endTime)) return false
  if (depositEndTime === 0 || depositEndTime > Date.now() / 1000) return true
  return false
}

export const isEnableToHarvest = (harvestStartTime): boolean => {
  return Date.now() / 1000 > harvestStartTime
}
